...
- [ ] 1.1 `requirements-pytorch.txt` — add `dots.tts` (0.3.1 is the version the
...
- [ ] 1.2 `setup.sh` — install dots.tts without upgrading torch (`--no-deps`, or
      install after torch and assert the pair). Verify `python -c "import
      dots_tts"` succeeds in the venv `run.sh` resolves.
- [ ] 1.3 Verify: on a host with the deployed venv, `import dots_tts` raises no
      `RuntimeError` about mismatched minors.

## 2. The engine

- [ ] 2.1 `engines.py` — add `DotsEngine(Engine)` beside `VoxcpmEngine`:
...
- [ ] 2.2 `engines.py` — `load()`: `DotsTtsRuntime.from_pretrained(model,
      ...
      the constants away.
- [ ] 2.3 `engines.py` — `unload()`: drop the runtime, `gc.collect()`,
      `_free_cuda()`, `_trim_ram()`, exactly as `VoxcpmEngine.unload` does.
- [ ] 2.4 `engines.py` — `_clone_kwargs(voice_dir, meta)`: always
...
- [ ] 2.5 `engines.py` — `generate()` returns `(np.float32[], self.sample_rate)`;
...
- [ ] 2.6 `engines.py` — `prepare_voice()` may stay a no-op, matching
      `VoxcpmEngine` (voice state is file paths). If reference-encoding caching is
      added later it belongs here, not in `generate`.

## 3. Registration

- [ ] 3.1 `server.py:33` — import `DotsEngine` alongside the other engines.
- [ ] 3.2 `server.py:129` — `_FOOTPRINTS["dots"] = 6_500_000_000`. **Measured
      ...
      checkpoint.
- [ ] 3.3 `server.py:146` — add `"dots": DotsEngine()` to `_ENGINES`. Do not
      change `POLYTTS_DEFAULT_ENGINE`; residency policy follows the existing
      `SOFT_PIN if name == DEFAULT_ENGINE` rule unchanged.
- [ ] 3.4 `server.py:912` — add `"dots"` to the `/voices` engine allowlist.

## 4. Clone-mode validation

- [ ] 4.1 `server.py` `/voices` handler — reject with 4xx, naming `ref_text`, a
...
- [ ] 4.2 Verify the negative and the positive: empty `ref_text` with
      `x_vector_only_mode=true` still registers.

## 5. Evidence — run these against a real server, not a mock

- [ ] 5.1 Register a dots voice from an existing `voices/*/voice.wav` +
      `ref_text`, both modes, and confirm two distinct `voice_id`s (the id hashes
      the mode).
- [ ] 5.2 `POST /tts/stream` for a dots voice: assert more than one chunk
      arrives, that the first bytes precede completion, and that
      `X-Sample-Rate: 48000`.
- [ ] 5.3 Determinism: synthesize the same `(text, voice_id)` twice, assert the
      PCM bodies are byte-identical, then evict and repeat to confirm a cache hit
      matches a fresh synthesis.
- [ ] 5.4 Residency: with dots resident, request a voxcpm voice; assert `GET
      /health` reports exactly one resident engine throughout, and that `POST
      /model/unload` returns the VRAM.
- [ ] 5.5 Regression: a `voxcpm` and a `qwen` voice registered before this change
      still synthesize, with unchanged `voice_id`s.
- [ ] 5.6 Record measured similarity, CER, TTFA and RTF for at least one
...
- [ ] 6.1 `README.md` — add `dots` to the engines list and the env-var table
...
- [ ] 6.2 `README.md` — record the clone-mode difference measured for this
